...
class CameraPickerViewerState extends State<CameraPickerViewer> {
  ...
  /// 播放控制器是否在播放
  bool get isControllerPlaying => videoController.isPlaying() ?? false;
  ...
  Future<void> initializeVideoPlayerController() async {
    try {
      videoController.addEventsListener(videoControllerListener);
      hasLoaded = true;
  ...
  /// 播放器的监听方法
  void videoControllerListener(BetterPlayerEvent event) {
    if (event.betterPlayerEventType == BetterPlayerEventType.play ||
        event.betterPlayerEventType == BetterPlayerEventType.pause) {
      final bool playing = isControllerPlaying;
  ...
  Future<void> playButtonCallback() async {
    try {
      if (isPlaying.value) {
        ...
        final duration = videoController.videoPlayerController?.value.duration;
        final position = videoController.videoPlayerController?.value.position;
